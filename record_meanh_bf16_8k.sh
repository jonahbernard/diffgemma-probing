#!/usr/bin/env bash
# meanH overlay data for bf16 at 8k canvases: 50 prompts x 300 seeds = 15000
# generations of 8000 tokens, recorded at BOTH temperature 0 and 0.6, against
# the bf16 server (port 8002). Run ./launch_bf16_probe.sh first.
#
...
#   runs/meanh50/bf16_8k_temp0.jsonl
#   runs/meanh50/bf16_8k_temp0.6.jsonl
#
# Override SEEDS (canvases/prompt), MAXTOK, or TEMPS:
#   SEEDS="0 1 2" TEMPS="0 0.6 1.0" ./record_meanh_bf16_8k.sh
set -euo pipefail
...
for TEMP in ${TEMPS:-0}; do
  ...
  MODEL=diffgemma-bf16 PORT="$PORT" \
    ...
    OUT=/app/diffgemma-probing/runs/meanh50/bf16_8k_temp${TEMP}.jsonl \
